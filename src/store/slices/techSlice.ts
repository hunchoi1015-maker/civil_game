...
export const createTechSlice: StateCreator<GameStore, [["zustand/immer", never]], [], TechSlice> = (set, get) => ({
  ...
  researchTech: (techId: string) => {
    ...
    set((state) => {
      ...
      if (!check.canResearch) {
          alert({ text: check.reason, type: 'warning' });
          return;
      }
      ...
      if (availableTrade < cost) {
          pendingToasts.push({ text: `사용 가능한 교역 토큰이 부족합니다. (비용: ${cost}, 사용 가능: ${availableTrade})`, type: 'warning' });
          return;
      }
      ...
      switch (techId) {
        ...
        case 'democracy':
          if (tech.tokensOnCard >= (tech.resourceAbility.maxTokens || 4)) {
              pendingToasts.push({ text: "이 기술 카드에 더 이상 화폐 토큰을 올릴 수 없습니다.", type: 'warning' }); return;
          }
          if (techId === 'pottery' && payload?.consumedResources) {
              ...
              if (!canPay) { pendingToasts.push({ text: "자원이 부족합니다.", type: 'warning' }); return; }
              ...
              if (total < 2) { pendingToasts.push({ text: "자원이 부족합니다.", type: 'warning' }); return; }
              costPaid = true;
          } else if (techId === 'printing_press') {
              if (player.resources.culture >= 5) { player.resources.culture -= 5; costPaid = true; } else { pendingToasts.push({ text: "문화 토큰이 부족합니다.", type: 'warning' }); return; }
          } else if (techId === 'democracy') {
              if (player.resources.trade >= 6) { player.resources.trade -= 6; costPaid = true; } else { pendingToasts.push({ text: "교역 토큰이 부족합니다.", type: 'warning' }); return; }
          }
          break;

        case 'writing':
        case 'communism':
          if (player.spies >= 1) { player.spies -= 1; costPaid = true; } else { pendingToasts.push({ text: "스파이가 부족합니다.", type: 'warning' }); return; }
          break;

        case 'currency':
        case 'chivalry':
        case 'metallurgy':
          if (hasEnoughLuxuryResource(player, 'spice', 1)) { consumeLuxuryResource(player, state.marketResources, 'spice', 1); costPaid = true; } else { pendingToasts.push({ text: "향료가 부족합니다.", type: 'warning' }); return; }
          break;

        case 'animal_husbandry':
        case 'construction':
        case 'finance':
          if (hasEnoughLuxuryResource(player, 'wheat', 1)) { consumeLuxuryResource(player, state.marketResources, 'wheat', 1); costPaid = true; } else { pendingToasts.push({ text: "밀이 부족합니다.", type: 'warning' }); return; }
          break;

        case 'horseback_riding':
        case 'monarchy':
          if (hasEnoughLuxuryResource(player, 'silk', 1)) { consumeLuxuryResource(player, state.marketResources, 'silk', 1); costPaid = true; } else { pendingToasts.push({ text: "비단이 부족합니다.", type: 'warning' }); return; }
          break;
        ...
        case 'steam_power':
          if (hasEnoughLuxuryResource(player, 'silk', 1) && state.steamPowerSource && payload?.x !== undefined && payload?.y !== undefined) {
              ...
              if (targetTile.terrain !== 'water') { 
                  pendingToasts.push({ text: "도착지는 반드시 물 타일이어야 합니다.", type: 'warning' }); 
              ...
              if (myUnitsOnTarget + sourceUnits.length > stackingLimit) {
                  pendingToasts.push({ text: `도착지 타일의 배치 한도(${stackingLimit}개)가 초과되어 순간이동할 수 없습니다.`, type: 'warning' });
                  state.steamPowerSource = null; return; 
              }
              ...
              pendingToasts.push({ text: "비단이 부족하거나 출발지/도착지가 올바르지 않습니다.", type: 'warning' }); 
              state.steamPowerSource = null; return;
          }
          break;
        ...
        case 'atomic_theory': 
          if (player.nuclearMaterial >= 1) {
              if (state.currentPhase === 'cityManagement' && tech.usedPhases?.includes('cityManagement')) { pendingToasts.push({ text: "이미 사용했습니다.", type: 'warning' }); return; }
              if (state.currentPhase === 'movement' && tech.usedPhases?.includes('movement')) { pendingToasts.push({ text: "이미 사용했습니다.", type: 'warning' }); return; }
              player.nuclearMaterial -= 1;
              if (!tech.usedPhases) tech.usedPhases = [];
              tech.usedPhases.push(state.currentPhase);
              costPaid = true;
          } else { pendingToasts.push({ text: "우라늄이 부족합니다.", type: 'warning' }); return; }
          break;
        ...
        case 'philosophy':
            if (payload?.consumedResources) {
                ...
                if (total < 3) { pendingToasts.push({ text: "자원이 부족합니다.", type: 'warning' }); return; }
                costPaid = true;
            }
            break;
        ...
        switch (techId) {
            ...
            case 'gunpowder':
                if (payload.targetType === 'wonder') {
                    ...
                    if (tp) { 
                        tp.invalidatedWonders = tp.invalidatedWonders || []; 
                        tp.invalidatedWonders.push(payload.targetWonder); 
                        success = true; 
                        pendingToasts.push({ text: "🗽 [불가사의 무효화 성공!]", type: 'success' });
                    }
                    ...
                    if (payload.targetType === 'random_card' && tp && tp.armyCards.length > 0) {
                        const dCard = tp.armyCards.splice(Math.floor(Math.random() * tp.armyCards.length), 1)[0]; 
                        success = true; 
                        pendingToasts.push({ text: `⚔️ [부대 파괴 성공: ${dCard.name}]`, type: 'success' });
                        ...
                        if (bIdx !== undefined && bIdx !== -1) { 
                            tp!.cities.find(c => c.id === payload.targetCityId)!.buildings.splice(bIdx, 1); 
                            success = true; 
                            pendingToasts.push({ text: "🏗️ [건물 파괴 성공!]", type: 'success' });
                        }
                    }
                } break;
            case 'steam_power':
                if (payload.x !== undefined && payload.y !== undefined && state.steamPowerSource) {
                    ...
                    success = true; state.steamPowerSource = null; 
                    pendingToasts.push({ text: "🚂 [증기력] 유닛 순간이동 성공!", type: 'success' });
                } break;
            case 'atomic_theory':
                if (payload.x !== undefined && payload.y !== undefined) {
                    ...
                    pendingToasts.push({ text: "☢️ [원자론] 핵 공격이 감행되었습니다!", type: 'error' });
                } else {
                    player.cities.forEach(city => { city.hasActedThisTurn = false; city.hasHarvestedCulture = false; }); 
                    pendingToasts.push({ text: "⚡ [원자론] 핵발전소 가동! 도시 행동 회복.", type: 'success' });
                } success = true; break;
            case 'metal_casting':
  ...
  grantFreeTech: (playerId: string, techId: string) => {
    set((state) => {
      ...
      if (player.technologies.some(t => t.id === techId)) return;
